...
export default function AssignmentSolvePage() {
  ...
  // красиво показываем многострочные строки
  ...
  const onlyNewlineDiffers = (aa, bb) =>
    normalizeNewlines(aa) === normalizeNewlines(bb);
  ...
  // стартовые заготовки, если поле пустое
  useEffect(() => {
    if (code.trim()) return;
  ...
  return (
    <Layout>
      ...
      <div className="grid lg:grid-cols-3 gap-6">
        ...
        {/* правая колонка: язык + код + запуск */}
          ...
          <Card>
            <div className="grid gap-3">
              ...
              <Button onClick={onSubmit} disabled={submitting || !code.trim()}>
                <Play size={16} />{' '}
                {submitting ? 'Отправляю…' : 'Отправить решение'}
              </Button>
            ...
            <Card>
              ...
              <div className="mt-4 grid sm:grid-cols-2 gap-4">
                {(result.cases ?? result.testCases ?? []).map((c, i) => {
                  const newlineOnly =
                    !c.passed && onlyNewlineDiffers(c.expected, c.actual);
                  return (
                    <div
                      key={i}
                      className="rounded-xl border border-slate-200 dark:border-slate-800 p-3 bg-white/60 dark:bg-slate-900/40"
