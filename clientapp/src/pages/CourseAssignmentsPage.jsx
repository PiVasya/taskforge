...
import { getAssignmentsByCourse, createAssignment, updateAssignment, updateAssignmentSort } from '../api/assignments';
...
export default function CourseAssignmentsPage() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        // нормализуем sort: если его нет — используем индекс
  ...
  const filtered = useMemo(() => {
    ...
    switch (sortMode) {
      case 'title_asc':  return [...s].sort((a, b) => byTitle(a, b, +1));
      case 'title_desc': return [...s].sort((a, b) => byTitle(a, b, -1));
      case 'created_asc':  return [...s].sort((a, b) => byCreated(a, b, +1));
      case 'created_desc': return [...s].sort((a, b) => byCreated(a, b, -1));
      case 'default':
      default:           return [...s].sort(bySort);
    }
  }, [items, q, sortMode]);
  ...
  // === перемещение в стандартном режиме (редактор) ===
  const swapByIndex = async (i, j) => {
    if (i < 0 || j < 0 || i >= filtered.length || j >= filtered.length) return;

    // найдём реальные индексы в исходном массиве items
    const a = filtered[i];
    const b = filtered[j];

    // оптимистично меняем sort локально
    ...
    try {
      // отправим на бек две операции
      await Promise.all([
        updateAssignmentSort(a.id, b.sort ?? j),
        updateAssignmentSort(b.id, a.sort ?? i),
      ]);
    } catch (e) {
      // если что-то пошло не так — просто перезагрузим список
      console.error(e);
      try {
        const data = await getAssignmentsByCourse(courseId);
        const norm = (data || []).map((x, k) => ({ ...x, sort: typeof x.sort === 'number' ? x.sort : k }));
        setItems(norm);
      } catch {}
    }
  };
    ...
    <Layout>
            ...
            <select
              ...
              onChange={(e) => setSortMode(e.target.value)}
               className="border rounded-lg px-3 py-2
                        ...
                        ocus:outline-none focus:ring-2 focus:ring-sky-500/60"
              title="Сортировка"
            >
              {SORT_OPTIONS.map((o) => (
      ...
      <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-5">
        {filtered.map((a, idx) => {
              ...
              <IfEditor>
                <div className="flex items-center gap-1">
                  <button
                    ...
                    title="Выше"
                    onClick={(e) => { e.preventDefault(); swapByIndex(idx, idx - 1); }}
                  >
                    <ArrowUp size={16} />
                  </button>
                  <button
          ...
          const CardInner = (
            <Card className={'transition hover:shadow-lg ' + (solved ? 'border-emerald-400/40 bg-emerald-500/5' : '')}>
              {/* Верхняя строка: заголовок слева, тулбар справа */}
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 grow">
                  <div className="flex items-center gap-2">
                    <div
                      className={'text-lg font-semibold truncate ' + (solved ? 'text-emerald-600' : '')}
                      title={a.title}
                    >
                      {a.title}
                    </div>
                ...
                {/* справа — мини-управление порядком в режиме редактора */}
                {EditorToolbar}
              </div>
            </Card>
          );
