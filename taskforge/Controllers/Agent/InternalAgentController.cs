    ...
    public sealed class InternalAgentController : ControllerBase
    {
        ...
        private async Task<object?> TryCreateHiddenDraftAssignmentAsync(AgentRun run, AgentRunArtifact artifact, DateTime now)
        {
            ...
            var existingDraft = await _db.TaskAssignments
                ...
                .ThenBy(x => x.Sort)
                .FirstOrDefaultAsync();
            if (existingDraft == null && sourceTaskIndex.HasValue && IsLearningBridgeDraftData(data))
            {
                var bridgeCandidates = await _db.TaskAssignments
                    ...
                    .OrderBy(x => x.Sort)
                    .ToListAsync();
                existingDraft = bridgeCandidates.FirstOrDefault(x => IsLikelySameLearningBridgeStep(x.Title, normalizedTitle, sourceTaskIndex.Value));
            }
