...
CONCEPT_KEYWORDS = {
    "output": ["print", "вывод", "напечат", "экран"],
    "input": ["input", "ввод", "считы", "прочита", "ввести"],
    "variables": ["переменн", "variable", "присво", "хран"],
    "comparison": ["сравн", "больше", "меньше", ">", "<", "равно"],
...
def assignment_text(assignment: Dict[str, Any]) -> str:
    values = [
        assignment.get("title"), assignment.get("Title"), assignment.get("name"),
        assignment.get("description"), assignment.get("Description"), assignment.get("text"),
        assignment.get("overview"), assignment.get("aiOverview"),
    ]
    return "\n".join(str(v) for v in values if v).lower()


def assignment_id(assignment: Dict[str, Any], fallback_index: int) -> str:
    return str(_first(assignment.get("id"), assignment.get("Id"), assignment.get("assignmentId"), f"assignment-{fallback_index + 1}"))


def assignment_title(assignment: Dict[str, Any], fallback_index: int) -> str:
    return str(_first(assignment.get("title"), assignment.get("Title"), assignment.get("name"), f"Задание {fallback_index + 1}"))
...
def build_course_digest(course_title: Optional[str], assignments: List[Dict[str, Any]]) -> Dict[str, Any]:
    ...
    for index, assignment in enumerate(assignments):
        ...
        for concept in concepts:
            concept_order.setdefault(concept, assignment_id(assignment, index))
        items.append({
            "id": assignment_id(assignment, index),
    ...
    return {
        ...
        "freshness": "payload-derived" if assignments else "missing-course-data",
    }
...
class ContextSupervisor:
    ...
    def build_snapshot(self, message: IncomingAgentMessage, allow_stale: bool = True) -> AgentContextSnapshot:
        ...
        assignments = _extract_assignments_from_payload(message.payload, message.selected_course)
        existing_digest = message.payload.get("courseDigest") or memory.get("courseDigest") or memory.get("lastCourseAnalysis")
        digest = existing_digest if isinstance(existing_digest, dict) and existing_digest.get("assignments") else build_course_digest(course_title, assignments)
        style = message.payload.get("styleProfile") or memory.get("styleProfile")
        if not isinstance(style, dict):
            style = build_style_profile(assignments)
        concept_map = message.payload.get("conceptMap") or memory.get("conceptMap")
        ...
        return AgentContextSnapshot(
            ...
            gap_map=message.payload.get("gapMap") if isinstance(message.payload.get("gapMap"), dict) else memory.get("lastGapAudit"),
