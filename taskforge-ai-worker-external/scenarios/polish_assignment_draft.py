...
class PolishAssignmentDraftScenario(Scenario):
    ...
    def run(self, context: AgentContextSnapshot, route: Optional[ScenarioRoute], previous_results: List[ScenarioResult]) -> ScenarioResult:
        ...
        user = (
            ...
            "Текст задания должен быть в стиле начальных задач TaskForge: длинная понятная обучалка, 'Следуй шагам:', прямые строки/конструкции, пояснения в скобках. Используй реальные переводы строк: вступление, строка 'Следуй шагам:', каждый пункт и финальная фраза должны быть отдельными строками.\n"
