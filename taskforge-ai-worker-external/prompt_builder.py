...
def _pre_anchor_notice(payload: Dict[str, Any]) -> str:
    ...
    if concept_low == "if":
        extra = " Для if это означает: без явного if/else в промежуточных условиях, пока задача именно готовит к первому if."
    else:
        extra = f" Для {concept} это означает: без явного {concept} в промежуточных условиях, пока задача именно готовит к первой встрече с темой."
    return f"Для такого запроса tasks должны образовывать подготовительную лесенку ДО первого {concept}: через понятные маленькие шаги, видимый результат и постепенное приближение к теме. Не перепрыгивай сразу к новой конструкции и не вводи её раньше времени.{extra}\n"


def _anchor_onboarding_notice(payload: Dict[str, Any]) -> str:
    concept = _anchor_label(payload)
    return f"Для такого запроса tasks должны образовывать именно обучающую лесенку по {concept}: первый маленький контакт с темой, затем 2-4 шага на наращивание сложности, примеры с видимым результатом и без сухих абстрактных мостиков.\n"
