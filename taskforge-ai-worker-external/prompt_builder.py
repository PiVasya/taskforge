...
def build_chat_turn_prompt(job: Dict[str, Any], payload: Dict[str, Any]) -> str:
    ...
    if _action_mode_raw == "single":
        ...
        _action_mode_instruction = (
            "ВАЖНО: Группируй несколько actions за один ход, если они логически связаны и ускоряют процесс. "
            "Например, если после аудита очевидно, что нужна инспекция, а потом план мостиков — верни все нужные шаги разом. "
            "Если пользователь просит 'проанализируй и сделай' — выполни и аудит, и подготовку плана, и генерацию в одном ходе, если данных хватает. "
            "Никогда не дублируй один и тот же action. Каждый action в массиве должен быть уникальным по name. "
            "Максимум 4 actions за ход. "
        )
    ...
    return (
        ...
        "memory — это долговременная память всей сессии: прошлые цели пользователя, вложения, уже выполненные действия и найденные сущности. Backend может автоматически продолжить агентный цикл на несколько внутренних шагов подряд, поэтому для больших запросов про анализ курса, поиск anchor, сборку плана мостиков и подготовку генерации выбери самый сильный первый action и не проси пользователя подтверждать обычные промежуточные шаги без необходимости. "
        ...
        "Если пользователь просит вывести, показать, перечислить или дать список уже существующих заданий курса, это inspect-intent: используй inspect_course_assignments и не подменяй такой запрос prepare_bridge_plan/show_bridge_plan/revise_bridge_plan. "
