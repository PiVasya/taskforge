...
def _pre_anchor_notice(payload: Dict[str, Any]) -> str:
    ...
    return f"Для такого запроса tasks должны образовывать подготовительную лесенку ДО первого {concept}: через понятные маленькие шаги, видимый результат и постепенное приближение к теме. Не перепрыгивай сразу к новой конструкции и не вводи её раньше времени.\n"
