...
for key in ('MINECRAFT_DEATH_COORDINATES_COST','MINECRAFT_DEATH_CHEST_COST',
            ...
            'AI_REMOTE_BROWSER_SESSION_IDLE_MINUTES','AI_REMOTE_BROWSER_SESSION_ABSOLUTE_MINUTES',
            'BROWSER_PIDS_LIMIT'):
    positive_int(key)
...
try:
    ...
    if int(env.get('AI_REMOTE_BROWSER_SESSION_IDLE_MINUTES','0')) > int(env.get('AI_REMOTE_BROWSER_SESSION_ABSOLUTE_MINUTES','0')):
        errors.append('AI_REMOTE_BROWSER_SESSION_IDLE_MINUTES cannot exceed AI_REMOTE_BROWSER_SESSION_ABSOLUTE_MINUTES')
