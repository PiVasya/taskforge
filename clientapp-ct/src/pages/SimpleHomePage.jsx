      ...
      <div className="flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        <div className="min-w-0 flex-1">
          <div className="text-sm font-black uppercase tracking-wide text-brand-700 dark:text-brand-300">
            Добавить номер прямо в {part.title}
          </div>
          ...
          <p className="mt-2 text-sm leading-6 text-brand-900/75 dark:text-brand-100/75">
            Пиши хоть {part.code}31, хоть {part.code}100. Номер создаётся как
            полноценный раздел, не как временная ссылка.
          </p>
          {invalid ? (
...
export default function SimpleHomePage() {
  ...
  const loadEditorTree = async () => {
    if (!canEdit || !isEditorMode) return;
    setBusy("tree");
    setError("");
    try {
      ...
      setError(
        getApiErrorMessage(e, "Не удалось загрузить разделы редактора."),
      );
    } finally {
      setBusy("");
    }
        ...
        <div className="mx-auto max-w-6xl px-4 py-8 md:py-12">
          <section className="mb-8 rounded-[2rem] border border-neutral-200/80 bg-white p-6 text-center shadow-soft dark:border-neutral-800 dark:bg-neutral-900 md:p-10">
            ...
            <p className="mx-auto mt-4 max-w-2xl text-lg text-neutral-600 dark:text-neutral-300">
              Без лишней платформы: номер → HTML-конспект → случайные задания по
              этому же номеру.
            </p>
          </section>
          ...
          {canEdit && isEditorMode ? (
            <div className="mb-6 rounded-[1.5rem] border border-brand-200 bg-brand-50 p-4 text-sm font-semibold text-brand-900 shadow-soft dark:border-brand-900 dark:bg-brand-950/30 dark:text-brand-100">
              ...
              Режим редактора включён. Плюс для новых A/B теперь находится прямо
              в нужной части, а не где-то внизу.
              <button
                type="button"
                onClick={loadEditorTree}
                disabled={busy === "tree"}
                  ...
                  <div className="grid grid-cols-3 gap-3 sm:grid-cols-5 md:grid-cols-6 lg:grid-cols-10">
                    {sections.map((section) => (
                      ...
                      >
                        ...
                        {canEdit && isEditorMode && !section.isDefault ? (
                          <div className="mt-2 rounded-full bg-brand-50 px-2 py-1 text-[10px] font-black uppercase tracking-wide text-brand-700 dark:bg-brand-950/40 dark:text-brand-200">
                            вручную
                          </div>
                        ) : null}
                      </Link>
