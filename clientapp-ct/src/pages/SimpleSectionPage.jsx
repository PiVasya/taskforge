...
function StatusPill({ status }) {
  if (status === "new") {
    return (
      <span className="rounded-full bg-sky-100 px-2.5 py-1 text-xs font-bold text-sky-800 dark:bg-sky-950/40 dark:text-sky-100">
        новое
      </span>
    );
  }
  if (status === "wrong") {
    return (
      <span className="rounded-full bg-red-100 px-2.5 py-1 text-xs font-bold text-red-800 dark:bg-red-950/40 dark:text-red-100">
        нужно повторить
      </span>
    );
  }
  return (
    <span className="rounded-full bg-emerald-100 px-2.5 py-1 text-xs font-bold text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-100">
      решено
    </span>
  );
}
...
function RandomTaskCard({ task, progressMap, onAnswered }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled)
          setError(
            e?.userMessage || e?.message || "Не удалось открыть задание.",
          );
      } finally {
  ...
  return (
    <article className="rounded-[1.75rem] border border-neutral-200/80 bg-white p-5 shadow-soft dark:border-neutral-800 dark:bg-neutral-900">
      ...
      <p className="mt-3 whitespace-pre-line text-lg font-semibold leading-7">
        {taskInfo.prompt}
      </p>
      {answerText ? (
      ...
      {result && (
        <div
          className={`mt-5 rounded-3xl border p-4 ${result.isCorrect ? "border-emerald-200 bg-emerald-50 dark:border-emerald-900 dark:bg-emerald-950/20" : "border-red-200 bg-red-50 dark:border-red-900 dark:bg-red-950/20"}`}
        >
          ...
          {resultExplanation ? (
            ...
            <p className="mt-3 text-sm text-neutral-600 dark:text-neutral-300">
              Для этого задания объяснение пока не заполнено.
            </p>
          )}
        </div>
      )}
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled)
          setError(
            e?.userMessage || e?.message || "Не удалось загрузить решения.",
          );
      } finally {
        if (!cancelled) setLoading(false);
      }
  ...
  return (
    <div className="mb-5 rounded-[1.75rem] border border-neutral-200 bg-white p-4 shadow-soft dark:border-neutral-800 dark:bg-neutral-900 md:p-5">
      ...
      {loading ? (
        ...
        <div className="mt-4 rounded-2xl border border-red-200 bg-red-50 p-3 text-sm text-red-800 dark:border-red-900 dark:bg-red-950/20 dark:text-red-100">
          {error}
        </div>
      ) : items.length === 0 ? (
                ...
                <div className="flex flex-wrap items-start justify-between gap-3">
                  <div>
                    <div className="text-xs font-bold uppercase tracking-wide text-neutral-500 dark:text-neutral-400">
                      {item.task?.sectionCode}
                    </div>
                    <div className="font-bold">{item.task?.title}</div>
                  </div>
                  <span
                    className={`rounded-full px-3 py-1 text-xs font-bold ${item.isCorrect ? "bg-emerald-100 text-emerald-800 dark:bg-emerald-950/40 dark:text-emerald-100" : "bg-red-100 text-red-800 dark:bg-red-950/40 dark:text-red-100"}`}
                  >
                    {item.isCorrect ? "правильно" : "неправильно"} ·{" "}
                    {item.scorePercent}%
                  </span>
                </div>
                <p className="mt-3 whitespace-pre-line text-sm leading-6 text-neutral-700 dark:text-neutral-200">
                  {item.task?.prompt}
                </p>
                <div className="mt-3 rounded-2xl bg-white p-3 text-sm dark:bg-neutral-900">
                  Твой ответ:{" "}
                  <span className="font-bold">
                    {answerToText(item.answerJson)}
                  </span>
                </div>
                <div className="mt-3 rounded-2xl bg-white p-3 text-sm leading-6 dark:bg-neutral-900">
                  ...
                  {why ? (
                    ...
                    <p className="text-neutral-500 dark:text-neutral-400">
                      Объяснение для этого задания пока не заполнено.
                    </p>
                  )}
                </div>
              </div>
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled)
          setError(
            e?.userMessage || e?.message || "Не удалось загрузить задания.",
          );
      } finally {
        if (!cancelled) setLoading(false);
      }
      ...
      <div className="mb-5 flex flex-wrap items-center justify-between gap-3">
        <div>
          ...
          <div className="mt-3 flex flex-wrap gap-2 text-xs font-bold text-neutral-600 dark:text-neutral-300">
            ...
            <span className="rounded-full bg-red-100 px-3 py-1 dark:bg-red-950/40">
              повторить: {stats.wrong}
            </span>
            <span className="rounded-full bg-emerald-100 px-3 py-1 dark:bg-emerald-950/40">
              решено: {stats.correct}
            </span>
          </div>
        </div>
        <div className="flex flex-wrap gap-2">
          ...
          <button
            type="button"
            onClick={() => chooseRandom(allTasks)}
            disabled={allTasks.length === 0 || loading}
...
export default function SimpleSectionPage({ sectionCode }) {
  ...
  const normalizedSectionCode = normalizeSectionCode(
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled)
          setError(
            e?.userMessage || e?.message || "Не удалось загрузить конспект.",
          );
      } finally {
        if (!cancelled) setLoading(false);
      }
