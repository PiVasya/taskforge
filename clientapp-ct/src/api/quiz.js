...
export async function submitQuizAttempt(
  taskId,
  answer,
  { clientAttemptId, timeSpentSeconds } = {},
) {
...
export async function updateQuizTask(taskId, payload) {
  const res = await api.put(
    `/api/admin/quiz/tasks/${encodeURIComponent(taskId)}`,
    payload,
  );
  return res.data;
}
...
export async function deleteQuizTasksBySection(params = {}) {
  const res = await api.delete("/api/admin/quiz/tasks/by-section", { params });
  return res.data;
}
