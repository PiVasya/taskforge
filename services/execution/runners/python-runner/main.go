...
func compileProgram(kind, code, cwd string, timeMs, memMb int) (*preparedProgram, *processResult) {
	...
	case "javascript":
		...
		return &preparedProgram{Cwd: cwd, Cmd: "node", Args: []string{fmt.Sprintf("--max-old-space-size=%d", heap), src}}, nil
	...
	case "python":
		...
		res := runCommand("python3", []string{"-I", "-B", "-m", "py_compile", src}, cwd, "", timeoutDuration(timeMs))
		if res.ExitCode != 0 {
			msg := strings.TrimSpace(res.Stdout + res.Stderr)
			if msg == "" {
		...
		return &preparedProgram{Cwd: cwd, Cmd: "python3", Args: []string{"-I", "-B", src}}, nil
