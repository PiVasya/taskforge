...
#define TF_DENY_ERRNO(error_code) (SECCOMP_RET_ERRNO | ((error_code) & SECCOMP_RET_DATA))
#define TF_DENY_ACTION TF_DENY_ERRNO(EPERM)
#define TF_ENOSYS_ACTION TF_DENY_ERRNO(ENOSYS)
                            ...
                            long arg4, long arg5, long arg6)
{
    ...
    return x0;
#else
#error Unsupported architecture for TaskForge sandbox
#endif
}
...
static long tf_parse_positive(const char *name, long fallback, long maximum)
{
...
{
    const long cpu_seconds = tf_parse_positive("TASKFORGE_LIMIT_CPU_SECONDS", 10, 120);
    const long file_mb = tf_parse_positive("TASKFORGE_LIMIT_FSIZE_MB", 16, 256);
    const long nofile = tf_parse_positive("TASKFORGE_LIMIT_NOFILE", 128, 1024);

    tf_set_limit(RLIMIT_CORE, 0);
    tf_set_limit(RLIMIT_CPU, (rlim_t)cpu_seconds);
    tf_set_limit(RLIMIT_FSIZE, (rlim_t)file_mb * 1024U * 1024U);
    tf_set_limit(RLIMIT_NOFILE, (rlim_t)nofile);
}

static int tf_is_image_profile(void)
{
    const char *profile = getenv("TASKFORGE_SANDBOX_PROFILE");
    return profile != NULL && strcmp(profile, "image") == 0;
}

static int tf_is_xserver_profile(void)
{
    const char *profile = getenv("TASKFORGE_SANDBOX_PROFILE");
    return profile != NULL && strcmp(profile, "xserver") == 0;
}

static void tf_install_filter(int image_profile, int allow_processes)
{
    const uint32_t self_pid = (uint32_t)tf_raw_syscall6(__NR_getpid, 0, 0, 0, 0, 0, 0);
    const uint32_t process_action = allow_processes ? SECCOMP_RET_ALLOW : TF_DENY_ACTION;
    const uint32_t clone3_action = allow_processes ? SECCOMP_RET_ALLOW : TF_ENOSYS_ACTION;

    struct sock_filter common_filter[] = {
        ...
        BPF_STMT(BPF_RET | BPF_K, process_action),
#endif
#ifdef __NR_execveat
        ...
        BPF_STMT(BPF_RET | BPF_K, clone3_action),
#endif
#ifdef __NR_clone
        ...
        TF_ALLOW_SELF_PID_SYSCALL(__NR_rt_sigqueueinfo, offsetof(struct seccomp_data, args[0]), self_pid),
#endif
#ifdef __NR_rt_tgsigqueueinfo
        TF_ALLOW_SELF_PID_SYSCALL(__NR_rt_tgsigqueueinfo, offsetof(struct seccomp_data, args[0]), self_pid),
#endif
#ifdef __NR_tkill
    ...
    struct sock_filter managed_network_filter[] = {
        BPF_STMT(BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, arch)),
#if defined(__x86_64__)
        BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, AUDIT_ARCH_X86_64, 1, 0),
#elif defined(__aarch64__)
    ...
    struct sock_filter image_network_filter[] = {
        BPF_STMT(BPF_LD | BPF_W | BPF_ABS, offsetof(struct seccomp_data, arch)),
#if defined(__x86_64__)
        BPF_JUMP(BPF_JMP | BPF_JEQ | BPF_K, AUDIT_ARCH_X86_64, 1, 0),
#elif defined(__aarch64__)
    ...
    struct sock_fprog common_program = {
        .len = (unsigned short)(sizeof(common_filter) / sizeof(common_filter[0])),
        .filter = common_filter,
    };
    if (tf_raw_syscall6(__NR_prctl, PR_SET_SECCOMP, SECCOMP_MODE_FILTER,
                        (long)(uintptr_t)&common_program, 0, 0, 0) != 0) {
        tf_fail();
    }

    struct sock_fprog network_program = image_profile
        ? (struct sock_fprog){
              .len = (unsigned short)(sizeof(image_network_filter) / sizeof(image_network_filter[0])),
              .filter = image_network_filter,
          }
        : (struct sock_fprog){
              .len = (unsigned short)(sizeof(managed_network_filter) / sizeof(managed_network_filter[0])),
              .filter = managed_network_filter,
          };
    if (tf_raw_syscall6(__NR_prctl, PR_SET_SECCOMP, SECCOMP_MODE_FILTER,
                        (long)(uintptr_t)&network_program, 0, 0, 0) != 0) {
        tf_fail();
    }
}

__attribute__((constructor(101)))
static void tf_sandbox_preload_init(void)
{
    const char *submission = getenv("TASKFORGE_SUBMISSION");
    if (submission == NULL || strcmp(submission, "1") != 0) {
        return;
    }
    ...
    tf_apply_limits();
    const int xserver_profile = tf_is_xserver_profile();
    tf_install_filter(tf_is_image_profile() || xserver_profile, xserver_profile);
}
