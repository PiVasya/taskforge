...
internal static partial class QuizTaskEndpoints
{
    private static WebApplication MapQuizEndpoints(WebApplication app)
    {
        ...
        app.MapGet("/api/quiz/tasks/{idOrSlug}", async (QuizDbContext db, ClaimsPrincipal user, string idOrSlug) =>
        {
            ...
            if (task == null || (!task.IsPublished && !canSeeDrafts)) return Microsoft.AspNetCore.Http.Results.NotFound();
            ...
            return Microsoft.AspNetCore.Http.Results.Ok(new QuizTaskDetailsDto(
        ...
        app.MapPost("/api/quiz/tasks/{taskId:guid}/attempts", [Authorize] async (QuizDbContext db, ClaimsPrincipal user, Guid taskId, [FromBody] SubmitQuizAttemptRequest req) =>
        {
            ...
            if (existingByClientAttempt != null)
            {
                ...
                return Microsoft.AspNetCore.Http.Results.Ok(ToResult(existingByClientAttempt, existingVersion.ExplanationJson, existingProgress));
            }

            var task = await db.Tasks.FirstOrDefaultAsync(x => x.Id == taskId && x.IsPublished);
            if (task == null) return Microsoft.AspNetCore.Http.Results.NotFound();
            ...
            return Microsoft.AspNetCore.Http.Results.Ok(ToResult(attempt, version.ExplanationJson, progress));
        });
        ...
        app.MapGet("/api/quiz/me/solutions", [Authorize] async (QuizDbContext db, ClaimsPrincipal user, string? sectionCode) =>
        {
            ...
            if (taskIds.Count == 0) return Microsoft.AspNetCore.Http.Results.Ok(Array.Empty<QuizSolutionDto>());
            ...
            if (latestAttempts.Count == 0) return Microsoft.AspNetCore.Http.Results.Ok(Array.Empty<QuizSolutionDto>());
        ...
        app.MapGet("/api/quiz/me/progress", [Authorize] async (QuizDbContext db, ClaimsPrincipal user, string? sectionCode) =>
        {
            ...
            return Microsoft.AspNetCore.Http.Results.Ok(progressEntities.Select(QuizProgressDto.FromEntity).ToList());
        });
