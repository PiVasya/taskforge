...
app.MapPut("/api/task-tests/{assignmentId:guid}/edit", async (Guid assignmentId, JsonElement payload, TasksDbContext db) => await SaveSpec(assignmentId, payload, db, kind: "test"));
...
app.MapPut("/api/math-tasks/{assignmentId:guid}/edit", async (Guid assignmentId, JsonElement payload, TasksDbContext db) => await SaveSpec(assignmentId, payload, db, kind: "math"));
...
app.MapPost("/api/assignments/{assignmentId:guid}/image-test/reference", async (Guid assignmentId, HttpRequest req, TasksDbContext db) =>
{
    var assignment = await db.Assignments.FindAsync(assignmentId);
    if (assignment == null) return Results.NotFound(new { message = "Задание не найдено.", code = "ASSIGNMENT_NOT_FOUND" });
...
static async Task<IResult> RenderImageCode(Guid assignmentId, ImageCodeRequest request, TasksDbContext db, IHttpClientFactory clients)
{
    var assignment = await db.Assignments.AsNoTracking().FirstOrDefaultAsync(x => x.Id == assignmentId);
    if (assignment == null) return Results.NotFound(new { message = "Задание не найдено.", code = "ASSIGNMENT_NOT_FOUND" });
    ...
    try
    {
        ...
        var response = await http.PostAsJsonAsync($"http://{runner}:8000/render/debug", new { source = request.Code ?? string.Empty, stdin = request.Input, timeoutSeconds = request.TimeoutSeconds ?? 20, debug = true });
...
static async Task<IResult> CompareImageCode(Guid assignmentId, ImageCodeRequest request, TasksDbContext db, IHttpClientFactory clients, bool submit, HttpContext? context = null, IConfiguration? cfg = null)
{
    ...
    try
    {
        ...
        var render = await http.PostAsJsonAsync($"http://{runner}:8000/render/debug", new { source = request.Code ?? string.Empty, stdin = request.Input, timeoutSeconds = request.TimeoutSeconds ?? 20, debug = true });
...
static async Task<IResult> CompareImageUpload(Guid assignmentId, HttpRequest req, TasksDbContext db, IHttpClientFactory clients)
{
    ...
    try
    {
        ...
        var response = await http.PostAsync($"http://image-analyzer:8000/compare?threshold={threshold.ToString(System.Globalization.CultureInfo.InvariantCulture)}", mp);
        var raw = await response.Content.ReadAsStringAsync();
...
static async Task<IResult> ReviewAttempt(Guid attemptId, string kind, Guid? userId, bool isAdmin, TasksDbContext db)
{
    ...
    if (kind == "math") return Results.Ok(new { attemptId = attempt.Id, attempt.TaskAssignmentId, courseId = assignment?.CourseId ?? Guid.Empty, courseTitle = "", assignmentTitle = assignment?.Title ?? "Задание", attempt.UserId, attempt.AttemptNumber, attempt.StartedAt, submittedAt = attempt.SubmittedAt, passPercent = 60, attempt.TotalScore, attempt.EarnedScore, attempt.ScorePercent, attempt.Passed, attempt.TimeExpired, allowReview = true, blocks = JsonPropArray(review, "blocks") });
    return Results.Ok(new { attemptId = attempt.Id, attempt.TaskAssignmentId, courseId = assignment?.CourseId ?? Guid.Empty, courseTitle = "", assignmentTitle = assignment?.Title ?? "Задание", attempt.UserId, attempt.AttemptNumber, attempt.StartedAt, submittedAt = attempt.SubmittedAt, passPercent = 60, totalQuestions = attempt.TotalUnits, correctQuestions = attempt.CorrectUnits, attempt.ScorePercent, attempt.Passed, attempt.TimeExpired, allowReview = true, questions = JsonPropArray(review, "questions") });
}
