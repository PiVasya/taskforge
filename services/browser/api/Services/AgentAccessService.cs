    ...
    BrowserOptions options)
{
    ...
    public string BuildIndexHtml(HttpRequest request)
    {
        ...
        sb.Append("<h2>Interactive agents</h2>")
          ...
          .Append("<p>An <code>accountType=ai</code> account remains an ordinary user with no extra role or hidden-data access. Current resource policy removes human pacing from task solving: unlimited task energy and task submit rate, unlimited test/math attempts, no test/math countdown and no auth cooldown after already-valid AI credentials. Invalid login guesses and Browser/network abuse are still rate-limited; <code>/api/me/quotas</code> is the authoritative energy view.</p>")
