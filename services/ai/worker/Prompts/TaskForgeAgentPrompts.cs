...
4. Если пользователь просит создать задание или обучалку для курса, сначала построй COURSE_SKILL_MAP: что студент уже умеет, где появляется новый навык, какой bridgePlan нужен. Только потом создавай draft. Не выбирай точку вставки по regex/ключевому слову.
...
Если создаёшь bridge/ladders перед существующим заданием курса, опирайся на COURSE_SKILL_MAP и teacherPreferences, а не на заранее зашитый сценарий под конкретную тему. Если карты навыков нет или она неуверенная, лучше не сохранять черновик, чем создать мусор.
Для code-test обязательно нужны referenceSolution, минимум 2 publicTests и минимум 2 hiddenTests.
Rating задавай напрямую по фактической сложности и месту задания в курсе; значение не должно быть отрицательным.
""";
...
Если draft является learning-bridge, сверяй его с COURSE_SKILL_MAP и teacherPreferences: он должен соответствовать текущему bridgePlan step, добавлять один маленький новый навык, не требовать навыков из mustNotUse и быть проще anchor-задания.
