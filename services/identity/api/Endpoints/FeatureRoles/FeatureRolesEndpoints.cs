...
internal static partial class IdentityApiEndpoints
{
    ...
    {
        app.MapGet("/api/admin/feature-roles", async (IdentityDbContext db) => Microsoft.AspNetCore.Http.Results.Ok(await db.FeatureRoles.AsNoTracking().OrderBy(x => x.Code).Select(x => new { x.Id, x.Code, title = x.Title, x.Description, x.IsActive }).ToListAsync()));

        app.MapGet("/api/admin/feature-roles/users", async (IdentityDbContext db, string? query, int limit = 50) =>
        {
            var rows = await SearchUsersAsync(db, query, null, false, "login", "asc", System.Math.Clamp(limit, 1, 200));
            ...
            return Microsoft.AspNetCore.Http.Results.Ok(rows.Select(u => ToAdminUserDto(u, roleRows.Where(r => r.UserId == u.Id).Select(r => r.Code).ToArray())).ToList());
        });
        ...
        app.MapPost("/api/admin/feature-roles", async (FeatureRoleRequest request, IdentityDbContext db) =>
        {
            ...
            if (string.IsNullOrWhiteSpace(code)) return Microsoft.AspNetCore.Http.Results.BadRequest(new { message = "Код роли обязателен.", code = "ROLE_CODE_REQUIRED" });
            if (await db.FeatureRoles.AnyAsync(x => x.Code == code)) return Microsoft.AspNetCore.Http.Results.Conflict(new { message = "Такая роль уже существует.", code = "ROLE_ALREADY_EXISTS" });
            ...
            return Microsoft.AspNetCore.Http.Results.Ok(new { role.Id, role.Code, title = role.Title, role.Description, role.IsActive });
        });
        ...
        app.MapPut("/api/admin/feature-roles/{id:guid}", async (Guid id, FeatureRoleRequest request, IdentityDbContext db) =>
        {
            ...
            if (role == null) return Microsoft.AspNetCore.Http.Results.NotFound(new { message = "Роль не найдена.", code = "ROLE_NOT_FOUND" });
            var nextCode = NormalizeRoleCode(request.Code ?? role.Code);
            if (!string.Equals(role.Code, nextCode, StringComparison.OrdinalIgnoreCase) && await db.FeatureRoles.AnyAsync(x => x.Code == nextCode)) return Microsoft.AspNetCore.Http.Results.Conflict(new { message = "Такая роль уже существует.", code = "ROLE_ALREADY_EXISTS" });
        ...
        {
            var role = await db.FeatureRoles.FindAsync(id);
            if (role == null) return Microsoft.AspNetCore.Http.Results.NoContent();
        ...
        app.MapPost("/api/admin/feature-roles/users/{userId:guid}/roles", async (Guid userId, RoleAssignRequest request, IdentityDbContext db) =>
        {
            ...
            var code = NormalizeRoleCode(request.Code);
            if (string.IsNullOrWhiteSpace(code)) return Microsoft.AspNetCore.Http.Results.BadRequest(new { message = "Роль не указана.", code = "ROLE_REQUIRED" });
            ...
            await db.SaveChangesAsync();
            return Microsoft.AspNetCore.Http.Results.Ok(ToAdminUserDto(user, await RolesForUser(db, user)));
        });
