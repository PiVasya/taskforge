...
Первый отдельный бизнесовый микросервис для TaskForge: перенос старых Python Telegram-ботов учителя и ученика на C#/.NET 10.

## Что внутри

- два Telegram-бота, как в старом проекте:
  - `TeacherBotHostedService`;
  - `StudentBotHostedService`;
- отдельная PostgreSQL-база микросервиса;
- EF Core + Npgsql;
- хранение изображений вопросов в существующем MinIO/S3;
...
- заготовка импортёра старой SQLite-базы `LegacySqliteImportService`.


## Миграции EF Core

В архиве специально **нет готовых миграций**. Их нужно сгенерировать после распаковки проекта.
...
dotnet ef migrations add InitialTelegramQuizSchema \
...
При запуске сервиса `Program.cs` через `DatabaseStartupService` сам применит сгенерированные миграции, если включено:
...
Это первый структурный перенос: сервис, БД, Docker, autobuild, базовые команды учителя/ученика и полный каркас старой схемы.
