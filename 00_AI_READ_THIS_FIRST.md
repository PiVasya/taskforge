...
- Текущая ветка `CustomMobTweaks` — версия `2.1.1` для Folia `26.1.2` / Java `25`. Не откатывать её к старой реализации `1.1.x` и не менять имя выходного JAR `CustomMobTweaks-*.jar` без прямой просьбы пользователя.
- Текущая ветка `TaskForgeLink` — версия `1.8.3` для Folia `26.1.2` / Java `25`. Сохранять полный hot-reload runtime через `/tflink reload`.
...
- Do not implement periodic link-status refreshes. Show death offers on respawn/link transition/join, after an explicit action result, or after reconnect.
