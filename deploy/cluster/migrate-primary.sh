...
for arg in "$@"; do
  case "$arg" in
    ...
    *) cluster_die "usage: sudo ./cluster/migrate-primary.sh --yes [--reset-cluster-data]" ;;
  esac
done
...
if [ -f "$rollback_marker" ]; then
  if [ "$reset_cluster_data" -ne 1 ]; then
    cluster_die "a previous bootstrap was rolled back. Retry only with: sudo ./cluster/migrate-primary.sh --yes --reset-cluster-data"
  fi
...
[ ! -f "$TASKFORGE_CLUSTER_RUNTIME/enabled" ] || cluster_die "cluster mode is already enabled; use cluster/status.sh instead"
...
"$CLUSTER_DIR/migrate-shared-volumes.sh"
...
if ! "$CLUSTER_DIR/wait-patroni.sh" primary 300; then
  cluster_compose logs --tail 200 postgres || true
  cluster_die "Patroni did not bootstrap the preferred primary. Run cluster/rollback-bootstrap.sh --yes to restore the old standalone stack."
fi
...
if [ -n "$missing" ]; then
  echo "$missing" >&2
  cluster_die "databases above were not restored. Inspect $backup_dir/restore.log and run cluster/rollback-bootstrap.sh --yes if necessary."
fi
...
"$CLUSTER_DIR/shared-state.sh" push || cluster_warn "shared state upload will be retried by the cluster agent"
...
echo "Next: run cluster/join-node.sh --yes on every remaining node."
