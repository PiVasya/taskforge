...
add_rule(){
  local source="$1" target="$2" target_ip="$3" target_port="$4" target_bucket="$5"
  local url out rc
  url="$(remote_url "$target_ip" "$target_port" "$target_bucket")"
  set +e
  out="$("$mcbin" replicate add "tf-$source/$target_bucket" --remote-bucket "$url" --replicate 'delete,delete-marker,existing-objects,metadata-sync' 2>&1)"
  rc=$?
  set -e
  if [ "$rc" -ne 0 ] && ! grep -Eqi 'already|exists|duplicate|same rule' <<<"$out"; then
    echo "$out" >&2
    cluster_die "cannot add MinIO replication rule $source -> $target for $target_bucket"
  fi
}
for source_row in "${nodes[@]}"; do
  IFS='|' read -r source source_ip source_port <<<"$source_row"
  for target_row in "${nodes[@]}"; do
    IFS='|' read -r target target_ip target_port <<<"$target_row"
    [ "$source" = "$target" ] && continue
    add_rule "$source" "$target" "$target_ip" "$target_port" "$bucket"
    add_rule "$source" "$target" "$target_ip" "$target_port" "$state_bucket"
  done
done
