...
if [ -x "$CLUSTER_DIR/../compose.sh" ]; then
  ...
  TASKFORGE_BACKUP="$TASKFORGE_ROOT/backup.sh"
else
  TASKFORGE_ROOT="$(cd "$CLUSTER_DIR/../.." && pwd)"
...
cluster_node_id(){ [ -f "$TASKFORGE_CLUSTER_RUNTIME/node-id" ] || cluster_die "local node id is not set; run sudo ./cluster/set-node.sh NODE_ID"; tr -d '\r\n' < "$TASKFORGE_CLUSTER_RUNTIME/node-id"; }
cluster_read_env(){
