...
export TASKFORGE_INVENTORY="${TASKFORGE_INVENTORY:-$ROOT_DIR/deploy/cluster/inventory.json}"
...
for local_env in "$ROOT_DIR/.runtime/cluster/local.env" "$ROOT_DIR/.runtime/cluster/cpu.env"; do
  if [ -f "$local_env" ]; then
    set -a
    . "$local_env"
    set +a
...
# N-node cluster mode is enabled by a runtime marker, not by adding dozens of
# HA variables to .env. TASKFORGE_CLUSTER_FORCE=1 is used only by bootstrap
# commands that need to start etcd before the database migration is activated.
CLUSTER_RUNTIME="$ROOT_DIR/.runtime/cluster"
CLUSTER_ENABLED=false
if [ -f "$CLUSTER_RUNTIME/enabled" ] || [ "${TASKFORGE_CLUSTER_FORCE:-0}" = "1" ]; then
  CLUSTER_ENABLED=true
  NODE_ENV="$CLUSTER_RUNTIME/node.env"
  [ -f "$NODE_ENV" ] || { echo "Missing $NODE_ENV. Run sudo deploy/cluster/set-node.sh NODE_ID first." >&2; exit 2; }
  set -a
  # Generated by clusterctl.py; values contain no shell code.
  . "$NODE_ENV"
  set +a
  export TASKFORGE_CLUSTER_ENABLED=true
  COMPOSE_FILES+=( -f deploy/cluster/compose.cluster.yaml )
fi
