...
root, with .NET 10, Node/npm, Go, gcc, pkg-config, the development libraries for
libpq/MariaDB Connector-C/SQLite, and Python 3 with PyYAML available. Python is for
the existing project/cluster checks, not the SQL runtime. See GO_RUNTIME.md for the
production compiler and native-library build contract:
...
credentials, private networks,
no host DB ports and dedicated disposable tmpfs. It runs real adapter, helper-isolation and AMQP wakeup tests and
...
replace the authoritative standalone r59 cluster package.
...
If Python r58 has already been deployed, do NOT publish the Go image into an
unattended rolling image channel before installing the r59 control/Compose bundle.
The r58 worker healthcheck runs Python and is incompatible with the Go image. During
a controlled maintenance window, use the existing cluster/watchtower controls to
prevent that premature worker rollout; install r59 on standbys first and current
Primary last, then roll the matching Go image set. Do not force-recreate the full
application stack or alter business volumes to switch a worker implementation.
If r58 was never deployed, there are no live Python SQL profiles to migrate.

After gates, control-bundle preparation and dev acceptance pass, build/push through
the repository workflow to
...
The full r59 archive is independent of both r57 and r58 folders. It requires Docker, existing
...
Use the SAME r59 package on A/B/C. Update standbys first and the actual current
Primary last; do not assume A is Primary. From each extracted r59 root:
