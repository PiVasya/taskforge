...
One Go sql-worker registers PostgreSQL, MySQL and SQLite adapters. The coordinator
and isolated helper are one executable with native client libraries via CGO.
Read GO_RUNTIME.md for the concrete security, build and lifecycle contract. PostgreSQL/MySQL
...
r59 runs SQL on the current full Primary. A/B support that role. C remains lite and
