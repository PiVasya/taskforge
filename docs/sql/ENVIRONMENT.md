...
Dev uses .runtime/sql-runtime.env; standalone r59 uses its stable config area's
...
published by default. r59 status/doctor reports optional SQL health without printing
secrets. A dedicated graphical metrics dashboard is not included.

## Go coordinator/helper

Compose sets GOMEMLIMIT=128MiB for the coordinator. This is a soft Go heap budget,
not the whole-container/native-memory limit. The child sets its own 64 MiB soft
heap budget, GOMAXPROCS=1, maximum 16 OS threads and parent-monitored RSS+swap
ceiling 256 MiB. Container pids_limit=256 covers the coordinator, drivers and
