...
} from '../attempts/attemptAnswerStore';
import MathTaskBlock from './MathTaskBlock';
import { recoverSubmittedAttempt, shouldRecoverSubmittedAttempt } from '../attempts/recoverSubmittedAttempt';
        ...
        <Card data-taskforge-automation-id="math-task-result" data-taskforge-agent-role="math-status" data-taskforge-agent-state={result?.passed ? 'passed' : 'failed'}>
          <div className="flex flex-wrap items-center justify-between gap-3">
            <div>
              ...
              <div className="text-sm text-neutral-600 dark:text-neutral-400">
                Порог: {result.passPercent}%. {result.timeExpired ? `⏱️ Время вышло. Неотвеченные пункты засчитаны как неправильные. ${result.passed ? '✅ Засчитано.' : '❌ Не засчитано.'}` : (result.passed ? '✅ Засчитано.' : '❌ Не засчитано.')}
              </div>
            </div>
            <div className="flex gap-3">
