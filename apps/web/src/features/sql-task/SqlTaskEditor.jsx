...
function SqlHelp({ children, label = 'Справка' }) {
  return <details className="sql-help sql-help--icon"><summary title={label} aria-label={label}>?</summary><div className="sql-help-body">{children}</div></details>;
}

const LIMIT_LABELS = {
  timeoutMs: 'Лимит времени, мс',
  maxRows: 'Максимум строк',
  previewRows: 'Строк в предпросмотре',
  maxBytes: 'Максимальный объём, байт',
  maxStatements: 'Максимум SQL-команд',
};

const SCHEMA_SETTING_LABELS = {
  constraintNamesMatter: 'Учитывать имена ограничений',
  defaultsMatter: 'Учитывать DEFAULT',
  indexesMatter: 'Учитывать индексы',
};
...
const SqlTaskEditor = forwardRef(function SqlTaskEditor({ assignmentId, onPublished }, ref) {
  ...
  const logicalEngines = logicalEngineProfiles(allProfiles, spec.targets, online);
  const upsertDatasetSummary = dataset => setDatasets(old => [dataset, ...old.filter(item => item.id !== dataset.id)]);
  const saveDataset = async () => {
    if (!name.trim()) throw new Error('Задай название набора данных.');
  ...
  const toggleEngine = (logical, checked) => {
    const profileId = logical.target?.engineProfileId || logical.preferredProfile?.id;
    if (!profileId) return;
    changeSpec({ targets: toggleEngineTargets(targets, profileId, checked) });
  };
  const upgradeEngine = logical => {
    if (!logical.target?.engineProfileId || !logical.preferredProfile?.id) return;
    changeSpec({ targets: replaceEngineTargetProfile(targets, logical.target.engineProfileId, logical.preferredProfile.id) });
  };
  ...
  return <div className="sql-task">
    <div className="sql-card"><div className="sql-toolbar"><h2>SQL / База данных</h2><span className="sql-badge">{view?.publishedVersionId ? 'Есть опубликованная версия' : 'Черновик'}</span><SqlHelp label="Справка по версиям SQL-задания"><p>Изменения создают новую версию набора данных и задания. Другие задания, использующие тот же набор, не изменяются. Ученики видят только опубликованную версию.</p></SqlHelp></div>
      {error && <div className="sql-error" role="alert">{error}</div>}{message && <div className="sql-ok" role="status">{message}</div>}
      <div className="sql-grid"><F label="Набор данных"><select disabled={busy} value={datasetId} onChange={e => void chooseDataset(e.target.value)}><option value="">+ Новый набор данных</option>{datasets.filter(d => !d.isArchived || d.id === datasetId).map(d => <option key={d.id} value={d.id}>{d.name}{d.isArchived ? ' [архив]' : ''}</option>)}</select></F>
        <F label="Версия набора"><select disabled={busy || !datasetId} value={versionId} onChange={async e => { const id = e.target.value; if (datasetDirty && !window.confirm('\u0421\u0431\u0440\u043e\u0441\u0438\u0442\u044c \u0438\u0437\u043c\u0435\u043d\u0435\u043d\u0438\u044f?')) return; setBusy(true); try { await loadDataset(datasetId,id); setSpecDirty(true); } catch(e) { setError(getApiErrorMessage(e)); } finally { setBusy(false); } }}><option value="">{'\u0415\u0449\u0451 \u043d\u0435 \u0441\u043e\u0445\u0440\u0430\u043d\u0451\u043d'}</option>{datasetInfo?.versions?.map(v => <option key={v.id} value={v.id}>v{v.version} / {v.contentHash?.slice(0,12)}</option>)}</select></F></div>
      <div className="sql-grid" style={{ marginTop: '.8rem' }}><F label="Название набора"><input maxLength={200} disabled={busy} value={name} onChange={e => { setName(e.target.value); setDatasetDirty(true); }} /></F><F label={'\u0414\u043e\u0441\u0442\u0443\u043f'}><select disabled={busy} value={scope} onChange={e => { setScope(e.target.value); setDatasetDirty(true); }}><option value="private">Мой набор данных</option><option value="editor-library">{'\u041e\u0431\u0449\u0430\u044f \u0431\u0438\u0431\u043b\u0438\u043e\u0442\u0435\u043a\u0430 \u0440\u0435\u0434\u0430\u043a\u0442\u043e\u0440\u043e\u0432'}</option></select></F></div>
      <F label="Описание набора"><input value={description} maxLength={2000} disabled={busy} onChange={e => { setDescription(e.target.value); setDatasetDirty(true); }} /></F>
      {datasetId && <button type="button" disabled={busy} onClick={() => { setDatasetId(''); setDatasetInfo(null); setVersionId(''); setScope('private'); setName(`${name} (копия)`); setDatasetDirty(true); setSpecDirty(true); }}>{'\u0421\u043e\u0437\u0434\u0430\u0442\u044c \u043d\u0435\u0437\u0430\u0432\u0438\u0441\u0438\u043c\u0443\u044e \u043a\u043e\u043f\u0438\u044e'}</button>}
    </div>
    <div className="sql-card"><SqlDatasetEditor value={doc} onChange={changeDoc} disabled={busy} /></div>
    <div className="sql-card"><div className="sql-toolbar"><h3>Движки и проверка</h3><SqlHelp label="Справка по режимам проверки"><p>SQL-команды выполняются по порядку до первой ошибки. В режиме результата сравнивается последний набор строк. CREATE DATABASE и DROP DATABASE доступны для PostgreSQL/MySQL и проверяются внутри изолированной учебной среды. Управление пользователями, сессией и внешними файлами недоступно.</p></SqlHelp></div>
      {!allProfiles.length && <div className="sql-error">SQL-сервис сейчас недоступен. Проверь состояние движков и повтори попытку.</div>}
      <div className="sql-status-list">{logicalEngines.map(item => <div className="sql-toolbar" key={item.engine}><Check label={item.displayName} value={item.selected} onChange={v => toggleEngine(item,v)} disabled={busy} /><span className={`sql-badge sql-badge--status ${item.online ? 'is-online' : 'is-offline'}`}><span className="sql-status-dot" aria-hidden="true" />{item.online ? 'Доступен' : 'Недоступен'}</span>{item.canUpgrade && <button type="button" disabled={busy} onClick={() => upgradeEngine(item)}>Использовать актуальную версию</button>}</div>)}</div>
      <F label="Режим проверки"><select value={spec.mode} disabled={busy} onChange={e => changeSpec({ mode:e.target.value,allowMultipleStatements:e.target.value !== 'result' })}><option value="result">Результат SELECT</option><option value="state">Состояние данных</option><option value="schema">Структура БД</option></select></F>
      <Check label="Разрешить несколько SQL-команд" value={spec.allowMultipleStatements} disabled={busy} onChange={v => changeSpec({ allowMultipleStatements:v })} />
      ...
      <details><summary>Расширенные настройки проверки</summary><Comparison value={spec.resultComparisonSettings} onChange={v => changeSpec({resultComparisonSettings:v})} disabled={busy} />
        ...
        {spec.mode === 'schema' && ['constraintNamesMatter','defaultsMatter','indexesMatter'].map(k => <Check key={k} label={SCHEMA_SETTING_LABELS[k]} value={spec.schemaCheckSettings[k]} disabled={busy} onChange={v => changeSpec({schemaCheckSettings:{...spec.schemaCheckSettings,[k]:v}})} />)}
        <div className="sql-grid">{[['timeoutMs',100,10000],['maxRows',1,1000],['previewRows',1,200],['maxBytes',1024,2097152],['maxStatements',1,50]].map(([k,min,max]) => <F key={k} label={LIMIT_LABELS[k]}><input type="number" min={min} max={max} value={spec.limits[k]} disabled={busy} onChange={e => changeSpec({limits:{...spec.limits,[k]:Number(e.target.value)}})} /></F>)}</div>
      </details>
    </div>
