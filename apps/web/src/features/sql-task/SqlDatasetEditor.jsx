...
export default function SqlDatasetEditor({ value, onChange, disabled = false }) {
  ...
  return <fieldset disabled={disabled} style={{ border: 0, padding: 0, minWidth: 0 }}>
    <div className="sql-toolbar"><h3>{'\u0422\u0430\u0431\u043b\u0438\u0446\u044b \u0438 \u0434\u0430\u043d\u043d\u044b\u0435'}</h3>
      <button type="button" disabled={tables.length >= 24} onClick={() => { let name = `table_${tables.length + 1}`; while (tables.some(t => t.name === name)) name += '_new'; mutate(next => { next.definition.tables.push(newTable(name)); }); setActive(tables.length); }}>{'+ \u0422\u0430\u0431\u043b\u0438\u0446\u0430'}</button></div>
    ...
    {table && <>
      ...
      <button type="button" style={{ marginTop: '.6rem' }} disabled={table.columns.length >= 64} onClick={() => mutate((next,t) => { let name = `column_${t.columns.length + 1}`; while(t.columns.some(c => c.name === name)) name += '_new'; t.columns.push(newColumn(name)); })}>{'+ \u041a\u043e\u043b\u043e\u043d\u043a\u0430'}</button>
