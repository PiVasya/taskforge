...
import { Box, CheckCircle2, Cloud, Cpu, Database, Globe2, Info, Network, Search, Server, ShieldCheck } from 'lucide-react';
import { array, bytes, containerState, containerTone, countPair, DASH, dateTime, duration, nodeTone, number, PHASE_LABELS, primaryRole, REASONS } from './clusterModel';
...
export default function ClusterNodeDetails({ node, nodes, onSelect }) {
  const [tab, setTab] = useState('overview');
  if (!node) return null;
  const reasons = array(node.healthReasons);
  return <section className="tf-cluster-panel" aria-label={`Детали сервера ${node.id}`}><div className="tf-cluster-node-selector" role="group" aria-label="Выбор сервера">{nodes.map(n => <button type="button" key={n.id} aria-pressed={node.id === n.id} onClick={() => onSelect(n.id)}><span className={`tf-cluster-dot is-${nodeTone(n)}`} />Сервер {n.id}<small>{primaryRole(n.role) ? 'PRIMARY' : n.role === 'standby' || n.role === 'replica' ? 'STANDBY' : '—'}</small></button>)}</div><div className="tf-cluster-detail-head"><div className="tf-cluster-title-group"><span className="tf-cluster-server-mark"><Server size={23} /></span><div><h2>Сервер {node.id}</h2><p>{roleLabel(node)} · {String(node.appProfile || DASH).toUpperCase()}{node.bundleRevision ? ` · Agent r${node.bundleRevision}` : ''}</p></div></div><div className="tf-cluster-address-group"><CopyValue value={node.network?.publicHost} label="Public" /><CopyValue value={node.network?.wireguardIp} label="WireGuard" /></div><Tag tone={nodeTone(node)} dot>{node.online ? node.telemetryFresh === false ? 'Данные устарели' : 'Онлайн' : 'Нет связи'}</Tag></div>
