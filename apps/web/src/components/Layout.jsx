...
function SideNavLink({
  ...
  const cls = `side-nav-link ${active ? "is-active" : ""} ${compact ? "is-compact" : ""} ${rail ? "is-rail" : ""}`;
    ...
    <Link to={to} className={cls} title={title}>
      ...
      {!rail && (
        <span className="min-w-0 flex-1">
          <span className={`side-nav-label ${compact ? "is-compact" : ""}`}>
            {label}
          </span>
        </span>
      )}
...
export default function Layout({
  ...
  const [colorTheme, setColorTheme] = useState(
    () => initialUi?.colorTheme || localStorage.getItem("colorTheme") || "pink",
  );
  const [mode, setMode] = useState(
    () => initialUi?.mode || localStorage.getItem("mode") || "dark",
  );
  ...
  const [bgFx, setBgFx] = useState(() =>
    typeof initialUi?.bgFx === "boolean"
      ? initialUi.bgFx
      : localStorage.getItem("bgFx") === "1",
  );
  const [fxMode, setFxMode] = useState(
    () => initialUi?.fxMode || localStorage.getItem("fxMode") || "random",
  );
  ...
  const [showSidebarToggle, setShowSidebarToggle] = useState(() => {
    ...
    return localStorage.getItem("showSidebarToggle") !== "0";
  });
    ...
    const applyFromStorage = () => {
      ...
      setShowSidebarToggle(
        typeof ui?.showSidebarToggle === "boolean"
          ...
          : localStorage.getItem("showSidebarToggle") !== "0",
      );
      setUiRev((r) => r + 1);
    };
  ...
  const { canEdit, isEditorMode, toggle, isAdmin, roles, hasRole } =
    useEditorMode();
  ...
  const primaryNav = [
    ...
    },
    {
      ...
      icon: GraduationCap,
      active: isActive("/courses") || isActive("/course"),
    },
    access && {
      to: "/my/solutions",
      label: "Мои решения",
    ...
    access &&
      canUseMinecraft && {
        ...
        icon: MessageSquare,
        active: isActive("/minecraft/chat"),
      },
  ].filter(Boolean);
    ...
    ? [
        {
          ...
          icon: Bot,
          active:
            isActive("/admin/ai") || isActive("/ai") || isActive("/agent"),
        },
        {
          to: "/admin/analytics",
          label: "Аналитика",
        ...
        },
        {
          ...
          icon: ListOrdered,
          active: isActive("/admin/solutions"),
        },
        {
          to: "/admin/badges",
          label: "Бейджи",
    ...
    ? [
        {
          ...
          icon: Link2,
          active: isActive("/admin/minecraft-links"),
        },
        {
        ...
        },
        {
          ...
          icon: Shield,
          active: isActive("/admin/feature-roles"),
        },
      ]
    : [];
  ...
  const currentViewTitle = (() => {
    ...
    if (currentPath.startsWith("/ai") || currentPath.startsWith("/agent"))
      return "AI-ассистент";
    if (currentPath.startsWith("/course") || currentPath.startsWith("/courses"))
      return "Курсы";
    ...
    <QuotaProvider enabled={!!access}>
      <div className="min-h-screen relative isolate">
        ...
        <main className={mainWrapClass}>
          <div
            className={`items-start gap-4 2xl:gap-6 ${access ? (sidebarCollapsed ? "xl:grid xl:grid-cols-[4.75rem,minmax(0,1fr)]" : "xl:grid xl:grid-cols-[15rem,minmax(0,1fr)] 2xl:grid-cols-[15.5rem,minmax(0,1fr)]") : ""}`}
          >
            {access && (
              <aside
                className={`dashboard-sticky-rail hidden xl:flex xl:flex-col gap-4 sticky top-24 self-start xl:max-h-[calc(100dvh-7rem)] ${sidebarCollapsed ? "w-[4.75rem]" : ""}`}
              >
                <div className={`card ${sidebarCollapsed ? "p-2" : "p-3"}`}>
                  ...
                  <div
                    className={`${sidebarCollapsed ? "space-y-1.5" : "mt-2 space-y-1.5"}`}
                  >
                    {primaryNav.map((item) => (
                      <SideNavLink
                        key={item.to}
                ...
                {isAdmin && (
                  <div className={`card ${sidebarCollapsed ? "p-2" : "p-3"}`}>
                    ...
                    <div
                      className={`${sidebarCollapsed ? "space-y-1.5 max-h-[52vh] overflow-y-auto" : "mt-2 space-y-1.5 max-h-[52vh] overflow-y-auto pr-1"}`}
                    >
