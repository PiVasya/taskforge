...
function defineDynamicMonacoThemes(monaco, editorStyle = 'color') {
  ...
  monaco.editor.defineTheme(`taskforge-dynamic-dark-${style}`, {
    ...
    colors: {
      ...
      'editor.selectionBackground': `#${style === 'mono' ? '2a3344' : withAlpha(accent, 0.30)}`,
      'editor.inactiveSelectionBackground': `#${style === 'mono' ? '202838' : withAlpha(accent, 0.18)}`,
      'editor.lineHighlightBackground': `#${style === 'mono' ? '141821' : withAlpha(accent, 0.10)}`,
...
export default function CodeEditor({
  ...
  const pickThemeName = useCallback(() => {
    ...
    return dark ? `taskforge-dynamic-dark-${editorStyle}` : `taskforge-dynamic-light-${editorStyle}`;
  }, [editorStyle]);
  ...
  const handleMount = useCallback((editor, monaco) => {
    ...
    if (wrapperRef.current && !roRef.current && typeof ResizeObserver !== 'undefined') {
      ...
      <Editor
        ...
        loading={(
          <div
            ...
            style={{ height, background: isDark ? '#0f1115' : 'rgb(var(--card))' }}
          >
            Загрузка редактора...
          </div>
        )}
